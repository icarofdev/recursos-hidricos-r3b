...
for (const name of ['index', 'login', 'register', 'forgot-password', 'reset-password']) {
 ...
 if (name === 'index') {
  content = content.replace('a cada 5 segundos', 'a cada 60 segundos')
   .replace('<option value="5000">', '<option value="60000">A cada 1 minuto</option><option value="300000">A cada 5 minutos</option><option value="5000">')
   .replace('<a class="skip-link"', '<div class="demo-banner" {{demoHidden}}>Demonstração local — os dados de telemetria são simulados.</div>\n    <a class="skip-link"');
 }
