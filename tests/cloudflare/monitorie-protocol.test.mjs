...
const compiled = await build({entryPoints:['cloudflare/monitorie/protocol.ts'],bundle:true,
 format:'esm',platform:'neutral',target:'es2022',write:false});
...
const makeGate = () => ({calls:0, delays:[], async reserve(){this.calls++;}, async defer(n){this.delays.push(n);}});
...
test('MonitorIE: login documentado envia JSON só ao destino fixo; nenhum segredo real', async () => {
 const gate=makeGate();
 const result=await loginMonitorie({username:'example@example.test',password:'FAKE_SECRET'},gate,async req=>{
 ...
 await assert.rejects(loginMonitorie({username:'x',password:'x'},gate,async()=>json({token:'x'})),code('MONITORIE_INVALID_DATA'));
});

test('MonitorIE: keys e latest usam DEVICE, X-Authorization e tipos preservados',async()=>{
 const requests=[]; const gate=makeGate();
 const client=new MonitorieReadClient(async()=>'FAKE_ACCESS',gate,async req=>{
  requests.push(req);
  assert.equal(req.headers.get('X-Authorization'),'Bearer FAKE_ACCESS');
  assert.equal(req.headers.get('Authorization'),null);
  assert.equal(req.method,'GET'); assert.equal(req.redirect,'error');
 ...
 assert.equal(requests.length,2); assert.equal(gate.calls,2);
...
test('MonitorIE: histórico mantém ms, limita por key e sinaliza possível truncamento',async()=>{
 const client=new MonitorieReadClient(async()=>'FAKE_ACCESS',makeGate(),async req=>{
  const q=new URL(req.url).searchParams;
  assert.equal(q.get('startTs'),String(stamp)); assert.equal(q.get('endTs'),String(stamp+1000));
  assert.equal(q.get('agg'),'NONE'); assert.equal(q.get('orderBy'),'ASC'); assert.equal(q.get('limit'),'2');
  return json({variavel_teste:[{ts:stamp+1000,value:2},{ts:stamp,value:1}]});
 });
 const data=await client.history(id,['variavel_teste','ausente'],stamp,stamp+1000,2);
 assert.equal(data.possiblyTruncated,true);
 assert.deepEqual(data.series.variavel_teste.map(x=>x.ts),[stamp,stamp+1000]);
 assert.deepEqual(data.series.ausente,[]);
});

test('MonitorIE: IDs/keys/limites inválidos não disparam chamadas',async()=>{
 let calls=0;const client=new MonitorieReadClient(async()=>'FAKE_ACCESS',makeGate(),async()=>{calls++;return json({});});
...
test('MonitorIE: falhas, resposta excessiva e 429 não vazam conteúdo nem causam retry automático',async()=>{
 for(const status of [401,403,404,429,500,302]){
  const gate=makeGate();let calls=0;
  const client=new MonitorieReadClient(async()=>'FAKE_ACCESS',gate,async()=>{
   calls++;return new Response('FAKE_SECRET',{status,headers:{'Retry-After':'120'}});
  });
  await assert.rejects(client.latest(id,['x']),e=>e.status===503&&!e.message.includes('FAKE_SECRET'));
  assert.equal(calls,1);
  if(status===429)assert.deepEqual(gate.delays,[120]);
 }
 for(const transport of [
  async()=>{throw new Error('FAKE_SECRET');},
 ...
 ]) await assert.rejects(new MonitorieReadClient(async()=>'FAKE_ACCESS',makeGate(),transport).latest(id,['x']),
...
test('MonitorIE: trava D1 disputa entre instâncias e mantém bloqueio após falha/Retry-After',async()=>{
 const mf=new Miniflare({modules:true,script:'export default {fetch(){return new Response("test")}}',
  compatibilityDate:'2026-03-01',d1Databases:{DB:'monitorie-test'},log:new Log(LogLevel.NONE)});
 try{
  const db=await mf.getD1Database('DB');
  await db.exec('CREATE TABLE settings (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at INTEGER NOT NULL)');
  const a=new D1MonitorieGate(db), b=new D1MonitorieGate(db);
  ...
  let sent=false;
  const client=new MonitorieReadClient(async()=>'FAKE_ACCESS',a,async()=>{sent=true;return json({});});
