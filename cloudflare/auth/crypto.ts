...
export function equal(a: string, b: string): boolean {
 const first = encoder.encode(a); const second = encoder.encode(b);
...
export function validatePassword(value: string, confirmation: string): void {
 if (encoder.encode(value).length < 10 || encoder.encode(value).length > 128 || !/[A-Za-z]/.test(value) || !/[0-9]/.test(value))
  throw new HttpError(422, 'WEAK_PASSWORD', 'Use uma senha de 10 a 128 caracteres, com letras e números.');
 if (!equal(value, confirmation)) throw new HttpError(422, 'PASSWORD_MISMATCH', 'A confirmação da senha não corresponde.');
}
...
export async function verifyPassword(password: string, stored: string | undefined, env: Env): Promise<boolean> {
 const parts = stored?.split('$') ?? [];
 const valid = parts.length === 5 && parts[0] === 'pbkdf2-sha256-pepper' && parts[1] === 'v1' && parts[2] === String(ITERATIONS)
  && /^[A-Za-z0-9_-]{22}$/.test(parts[3]) && /^[a-f0-9]{64}$/.test(parts[4]);
