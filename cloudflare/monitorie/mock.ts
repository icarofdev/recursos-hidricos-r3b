...
function reading(scope: TelemetryScope, time: number): Reading {
 const level = Math.round((65 + 14 * Math.sin(time / 3600)) * 100) / 100;
 return {id: scope.deviceId, distancia: 100 - level, nivel: level, volume: level * 20, rssi_wifi: -58, timestamp: new Date(time * 1000).toISOString()};
}
export class MockMonitorie implements MonitorieAdapter {
 async snapshot(scope: TelemetryScope): Promise<Snapshot> {
  const time = now();
  return {simulated: true, device: {id: scope.deviceId, type: scope.deviceType || 'SM-WU', status: 'online', last_seen: new Date(time * 1000).toISOString(), offline_after_seconds: scope.offlineAfterSeconds}, data: reading(scope, time)};
 }
 async history(scope: TelemetryScope, since: number, limit: number): Promise<Reading[]> {
  const end = now(); const start = Math.max(since, scope.linkedAt);
