...
export interface TelemetryPoint { ts: number; value: unknown }
...
const badInput = () => new HttpError(503, 'MONITORIE_NOT_CONFIGURED', 'Confirme os identificadores e variáveis da MonitorIE.');
...
function keysParam(keys: readonly string[]): string {
 if (keys.length < 1 || keys.length > 32 || new Set(keys).size !== keys.length
  || keys.some(key => typeof key !== 'string' || !key.trim() || key.length > 255 || /[,\x00-\x1f]/.test(key))) throw badInput();
 return keys.join(',');
}
...
async function requestJson(path: string, init: RequestInit, gate: RequestGate,
 transport: Transport, maxBytes = MAX_BYTES): Promise<unknown> {
 await gate.reserve();
 const controller = new AbortController();
 const timer = setTimeout(() => controller.abort(), 10000);
 try {
  const response = await transport(new Request(ORIGIN + path,
   {...init, redirect: 'error', signal: controller.signal}));
  if (response.status === 429) {
   const header = response.headers.get('Retry-After');
   const raw = header && /^\d+$/.test(header) ? Number(header)
  ...
  if (response.status === 401 || response.status === 403)
   throw new HttpError(503, 'MONITORIE_ACCESS_DENIED', 'Acesso à MonitorIE indisponível. Verifique a autorização.');
  ...
  try {
   while (true) {
    ...
    if (length > maxBytes) { await reader.cancel(); throw invalid(); }
...
export async function loginMonitorie(credentials: {username: string; password: string}, gate: RequestGate,
 transport: Transport = request => fetch(request)): Promise<{token: string; refreshToken: string}> {
 if (!credentials.username || !credentials.password) throw badInput();
 const data = await requestJson('/api/auth/login', {method: 'POST',
  headers: {'Content-Type': 'application/json', Accept: 'application/json'},
  body: JSON.stringify({username: credentials.username, password: credentials.password})}, gate, transport, 32768);
 if (!object(data) || typeof data.token !== 'string' || !data.token
  || typeof data.refreshToken !== 'string' || !data.refreshToken) throw invalid();
...
export class MonitorieReadClient {
 ...
 #gate: RequestGate;
 #transport: Transport;
 constructor(accessToken: () => Promise<string>, gate: RequestGate, transport: Transport = request => fetch(request)) {
  this.#accessToken = accessToken; this.#gate = gate; this.#transport = transport;
 }
 async #get(path: string): Promise<unknown> {
  const token = await this.#accessToken();
  if (!token || /\s/.test(token)) throw badInput();
  return requestJson(path, {method: 'GET', headers: {'X-Authorization': `Bearer ${token}`, Accept: 'application/json'}},
   this.#gate, this.#transport);
 }
 async keys(externalId: string): Promise<string[]> {
  ...
  if (!Array.isArray(data) || data.length > 1000 || data.some(key => typeof key !== 'string' || key.length > 255)) throw invalid();
  return data;
 }
 ...
 async history(externalId: string, keys: readonly string[], startMs: number, endMs: number,
  limit: number): Promise<{series: TelemetrySeries; possiblyTruncated: boolean}> {
  timestampMsToIso(startMs); timestampMsToIso(endMs);
  if (startMs > endMs || !Number.isInteger(limit) || limit < 1 || limit > 2000) throw badInput();
  const path = `/api/plugins/telemetry/DEVICE/${deviceId(externalId)}/values/timeseries`;
  const query = new URLSearchParams({keys: keysParam(keys), startTs: String(startMs), endTs: String(endMs),
   limit: String(limit), agg: 'NONE', orderBy: 'ASC', useStrictDataTypes: 'true'});
...
export class D1MonitorieGate implements RequestGate {
 ...
 async reserve(): Promise<void> {
  const result = await this.db.prepare(`INSERT INTO settings(key,value,updated_at)
   VALUES ('monitorie:next-request-ms', CAST((unixepoch('now') + 70) * 1000 AS TEXT), unixepoch('now'))
   ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at
   WHERE CAST(settings.value AS INTEGER) <= unixepoch('now') * 1000`).run();
 ...
 async defer(seconds: number): Promise<void> {
  await this.db.prepare(`UPDATE settings SET value=CAST(MAX(CAST(value AS INTEGER),
   (unixepoch('now') + ?) * 1000) AS TEXT) WHERE key='monitorie:next-request-ms'`)
   .bind(Math.ceil(Math.max(70, seconds))).run();
 }
}
