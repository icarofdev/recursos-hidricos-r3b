...
export class MonitorieSMWUAdapter implements SMWUAdapter {
 ...
 async snapshot(_scope: TelemetryScope): Promise<Snapshot> { return this.unavailable(); }
 async history(_scope: TelemetryScope, _since: number, _limit: number): Promise<Reading[]> { return this.unavailable(); }
 private unavailable(): never {
  void this.env;
  throw new HttpError(503, 'MONITORIE_NOT_CONFIGURED', 'A conexão com a Monitorie para SM-WU ainda não foi configurada. Sua conta continua disponível.');
 }
}
...
export class MonitorieSMWAAdapter implements SMWAAdapter {
 constructor(private readonly env: Env) {}
 async snapshot(_scope: TelemetryScope): Promise<Snapshot> { return this.unavailable(); }
 async history(_scope: TelemetryScope, _since: number, _limit: number): Promise<SMWAReading[]> { return this.unavailable(); }
 private unavailable(): never {
  void this.env;
  throw new HttpError(503, 'MONITORIE_NOT_CONFIGURED', 'A conexão com a Monitorie para SM-WA ainda não foi configurada. Sua conta continua disponível.');
 }
}
