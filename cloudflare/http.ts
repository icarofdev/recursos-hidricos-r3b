...
export function secret(env: Env, name: 'SESSION_SECRET' | 'PASSWORD_PEPPER'): string {
 ...
 if (!value || value.length < 32) throw new HttpError(503, 'CONFIGURATION_REQUIRED', 'Serviço ainda não configurado.');
 return value;
}
export function method(c: Context, allowed: string[]): void {
 if (!allowed.includes(c.request.method)) throw new HttpError(405, 'METHOD_NOT_ALLOWED', 'Método não permitido.');
}
...
export async function body(c: Context, allowed?: string[]): Promise<Record<string, unknown>> {
 ...
 try {
  while (true) {
   ...
   if (size > 16384) { await reader.cancel(); throw new HttpError(413, 'PAYLOAD_TOO_LARGE', 'Conteúdo maior que o permitido.'); }
   chunks.push(value);
  }
 ...
 if (!data || typeof data !== 'object' || Array.isArray(data)) throw new HttpError(400, 'INVALID_JSON', 'Envie um objeto JSON.');
 if (allowed && Object.keys(data).some(key => !allowed.includes(key))) throw new HttpError(422, 'UNKNOWN_FIELD', 'Campo não reconhecido.');
 return data;
}
export function string(data: Record<string, unknown>, field: string, max: number): string {
 const value = data[field];
 if (typeof value !== 'string' || [...value].length > max) throw new HttpError(422, 'INVALID_FIELD', `O campo ${field} é inválido.`);
 return value;
}
...
export function queryInt(c: Context, key: string, fallback: number, min: number, max: number): number {
 const text = c.url.searchParams.get(key); if (text === null) return fallback;
 const n = Number(text);
 if (!/^\d+$/.test(text) || !Number.isSafeInteger(n) || n < min || n > max) throw new HttpError(422, 'INVALID_QUERY', 'Parâmetro fora da faixa permitida.');
 return n;
}
...
export function secureResponse(response: Response, c: Context): Response {
 ...
 return new Response(c.request.method === 'HEAD' ? null : response.body, {status: response.status, headers});
}
